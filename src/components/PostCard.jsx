...
const PostCard = ({ 
  ...
  if (content !== prevProps.content || 
    ...
    setPrevProps({ content, hashtags, image_url, initialLikeCount, initialCommentCount });
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener(`post-like-update-${id}`, handleLikeUpdate);
  }, [id]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener(`post-update-sync-${id}`, handlePostUpdate);
  }, [id]);

  // Fallback for avatar
  const avatar = author_image || `https://ui-avatars.com/api/?name=${encodeURIComponent(author_name || 'User')}&background=0a66c2&color=fff`;
  ...
  useEffect(() => {
    ...
    const visibilityThreshold = Math.min(100, Math.max(10, minVisibilityPct)) / 100;
  ...
  if (isEditing) {
    return (
      <article id={`post-${id}`} className="post-card glass" style={{ marginBottom: '24px', overflow: 'hidden', padding: '20px', borderRadius: 'var(--border-radius-md)', borderLeft: '4px solid var(--primary-blue)' }}>
        <div className="post-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            ...
            <div className="post-author-info">
              <h3>Editing Post</h3>
              <p className="author-headline" style={{ color: 'var(--primary-blue)', fontWeight: '600' }}>Modify your thoughts</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleEditSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          {editImageUrl.trim() ? (
            <div style={{ position: 'relative', borderRadius: 'var(--border-radius-md)', overflow: 'hidden', border: '1px solid var(--border-color)', marginTop: '8px' }}>
              ...
              <div style={{ position: 'absolute', right: '10px', top: '10px', display: 'flex', gap: '8px' }}>
                <button 
                  ...
                  style={{ background: 'rgba(0,0,0,0.6)', color: 'white', border: 'none', borderRadius: '4px', padding: '6px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', fontWeight: '600' }}
                >
                ...
                <button 
                  ...
                  style={{ background: 'rgba(239, 68, 68, 0.8)', color: 'white', border: 'none', borderRadius: '4px', padding: '6px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.75rem', fontWeight: '600' }}
                >
            ...
            <button 
              type="button" 
              onClick={() => fileInputRef.current && fileInputRef.current.click()}
              style={{ padding: '12px', border: '1px dashed var(--border-color)', borderRadius: 'var(--border-radius-md)', background: 'rgba(0,0,0,0.02)', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontSize: '0.85rem', fontWeight: '600', transition: 'all 0.2s' }}
              onMouseOver={(e) => { e.currentTarget.style.background = 'var(--primary-light)'; e.currentTarget.style.color = 'var(--primary-blue)'; }}
              onMouseOut={(e) => { e.currentTarget.style.background = 'rgba(0,0,0,0.02)'; e.currentTarget.style.color = 'var(--text-muted)'; }}
            >
              <i className="fa-regular fa-image" style={{ fontSize: '1.1rem' }}></i> Upload Device Photo
            </button>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '12px', borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
            <button 
              ...
              style={{ padding: '8px 18px', borderRadius: '20px', border: 'none', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer', fontWeight: '600', fontSize: '0.85rem' }}
            >
              Cancel
            </button>
            <button 
  ...
  return (
    <article id={`post-${id}`} className="post-card glass" style={{ marginBottom: '24px', overflow: 'hidden', transition: 'border-color 0.5s ease, transform 0.3s ease' }}>
      ...
      <div className="post-stats" style={{ display: 'flex', gap: '16px', marginTop: '16px', paddingBottom: '12px', borderBottom: '1px solid var(--border-color)', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
        <span><i className="fa-solid fa-thumbs-up" style={{ color: 'var(--primary-blue)' }}></i> {likeCount} Likes</span>
        <span>{commentCount} Comments</span>
      </div>

      <div className="post-actions" style={{ display: 'flex', justifyContent: 'space-around', paddingTop: '12px', paddingBottom: '12px' }}>
        <button 
        ...
        >
          <i className={isLiked ? "fa-solid fa-thumbs-up" : "fa-regular fa-thumbs-up"}></i> {isLiked ? 'Liked' : 'Like'}
        </button>
        <button 
      ...
      {showComments && (
        <div className="comments-section" style={{ padding: '16px', background: 'rgba(255,255,255,0.4)', borderTop: '1px solid var(--border-color)' }}>
          <form onSubmit={handleCommentSubmit} style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
            <input 
              type="text" 
              value={newComment} 
              onChange={e => setNewComment(e.target.value)} 
              placeholder="Add a comment..." 
              style={{ flex: 1, padding: '10px 16px', borderRadius: '24px', border: '1px solid var(--border-color)', outline: 'none', background: 'white' }}
            />
            <button type="submit" disabled={!newComment.trim()} style={{ background: 'var(--primary-blue)', color: 'white', border: 'none', borderRadius: '24px', padding: '8px 20px', cursor: newComment.trim() ? 'pointer' : 'not-allowed', opacity: newComment.trim() ? 1 : 0.6, fontWeight: '600' }}>Post</button>
          </form>
          ...
          {loadingComments ? (
            ...
            <div className="comments-list" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {comments.length === 0 && (
                <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.85rem', padding: '8px 0' }}>No comments yet. Be the first!</div>
              )}
            </div>
          )}
        </div>
